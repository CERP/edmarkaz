import React from 'react'
import '../style.css'

// eslint-disable-next-line react/display-name
export default () => <div>
	<div className="tabs-banner">
		<img
			className="banner-img"
			src="https://lh6.googleusercontent.com/YxeFoAXAWXGmDb6Sl20CcC-b2YXAVLNHQt4p5M6hp9OVNn-TIcbAF9ztTnkD8u4KMtBU13NWTZNvgp9dwZgF4ecVfwhrEX0ruHGFrpUtuUN_duCRsDccT0fCeK4n5mSILESHjzf5"
		/>
	</div>
	<div className="article-page">
		<h1>Strong Leaders Make Great Schools </h1>
		<div className="urdu">
			<h1>پُرعزم رہنما عظیم اسکول بناتے ہیں</h1>
		</div>

		<p>School leaders are critical in developing high quality schools. If each school leader is responsible for creating a school where the learning environment is rich, then the educational system as a whole can be transformed, and with it so can the future of a nation</p>
		<div className="urdu">
			<p>اعلیٰ معیار کے اسکولوں کی بہتر ترویج میں اسکول قائدین کا کردار بہت اہم ہے۔ اگر ہر اسکول کا رہنما خود کو ایسے اسکول کی تشکیل کا ذمہ دار سمجھے جہاں سیکھنے کا ماحول کوشگوار ہو، تو پھر مجموعی طور پر تعلیمی نظام کو تبدیل کیا جاسکتا ہے، اور اس کے ساتھ ہی کسی قوم کا مستقبل بھی بدل سکتا ہے۔</p>
		</div>

		<p>Research shows that school leaders have a huge impact on student learning: studies have found that a single school principal can account for up to 25% of the impact that schools have on student learning and test scores. Changes in the practices of school principals can make a big difference in how much students learn and good schools generally have great school leaders.</p>
		<div className="urdu">
			<p>تحقیقات سے پتہ چلتا ہے کہ اسکول کے رہنماؤں کا طلباء کی تعلیم پر بہت زیادہ اثر پڑتا ہے: مطالعہ سے معلوم ہوا ہے کہ اسکول کے طالب علموں کی تعلیم اور امتحانی نمبروں پر پڑنے والے اثرات کا اسکول پرنسپل اکیلا ہی 25 فیصد حصہ بن سکتا ہے۔ اسکول کے پرنسپلز کے طریق کار میں ہونے والی تبدیلیوں سے اس بات میں بڑا فرق پڑ سکتا ہے کہ عام طور پر طلباء کتنا سیکھتے ہیں۔ اسکولوں کو بہترین سطح تک لے جانے کے پیچھے عموماً بہت عظیم رہنماؤں کا ہاتھ ہوتا ہے۔ </p>
		</div>

		<p>The world is changing everyday, faster than ever before. As a result, we need to preparing our children for a future that we may not even be able to predict. They will need skills that will equip them for life in the 21st century, commonly referred to as “21st-Century Skills”.</p>
		<h5>What roles can school leaders play to improve their schools?</h5>

		<div className="urdu">
			<h5>اسکول رہنما اپنے اسکولوں کی بہتری کے لئے کیا کردار ادا کرسکتے ہیں؟</h5>
		</div>

		<ol>
			<li><b> Setting Direction and Defining Goals </b></li>
			<p>School leaders need to decide what the focus of their school will be and what are their goals. They need to define these and communicate them to teachers and staff, as well as to parents and even students. It is up to the school leader to constantly remind the school community of these goals and make sure that they are being met. </p>
			<p>School Leaders can start setting the direction for their school by making a School Improvement Plan, which lays out their goals for the year.</p>

			<div className="urdu">
				<ol>
					<li><b> سمت طے کرنا اور اہداف ومقاصد کی وضاحت کرن</b></li>
					<p>	اسکول قائدین/رہنماؤں کو یہ فیصلہ کرنے کی ضرورت ہے کہ ان کے اسکول کی اصل توجہ کس چیز پر ہوگی اور ان کے مقاصد کیا ہوں گے۔ انہیں ضرورت ہے کہ وہ یہ مقاصد واضح کریں نیز اساتذہ اور عملے کے ساتھ ساتھ والدین اور یہاں تک کہ طلباء کو بھی ان مقاصد سے آگاہ کریں۔ اسکول قائد کی ذمہ داری ہے کہ وہ اسکول کمیونٹی کو بار بار یہ مقاصد یاد دلائیں اور اس بات کو یقینی بنائیں کہ ان مقاصد کی تکمیل ہو رہی ہے۔ </p>
					<p>اسکول قائدین اسکول کی بہتری کا ایسا خاکہ بنا کر اپنے اسکول کی سمت متعین کرنا شروع کرسکتے ہیں جس میں سال بھر کے اہداف ومقاصد بیان کیے گئے ہوں۔ </p>
				</ol>
			</div>

			<li><b>Developing People</b></li>
			<p>A school is only as effective as its teachers. It is the school leader’s job to ensure that teachers have content-knowledge as well as knowledge of pedagogy, or the instructional practices and teaching methods required for different subjects and age groups. Teachers need to be trained, coached, fully supported, and have opportunities to collaborate with each other and the administration to develop the knowledge and skills they need to meet the school’s goals. School leaders also need to monitor and evaluate teachers, to give them feedback in a positive manner on how to improve. School leaders are responsible for providing teachers with the resources and guidance they need to meet the school’s goals. </p>

			<div className="urdu">
				<ol start={2}>
					<li><b> ترقی پذیر لوگ</b></li>
					<p>	ایک اسکول صرف اتنا ہی مؤثر ہے جتنا کہ اس کے اساتذہ ۔ اسکول لیڈر کو یہ یقینی بنانا ہے کہ اساتذہ کو مشمولات کے ساتھ ساتھ تدریسی مواد کا علم بھی ہو، نیز ان کو درس و تدریس کی ایسی مشقوں اور طریقہ کار کا بھی علم ہو جس کی مختلف مضامین میں اور مختلف عمر کے گروپوں کو ضرورت پڑتی ہے۔ ضرورت اس امر کی ہے کہ اساتذہ کو تربیت دی جائے، ان کی راہنمائی کی جائے، ان کی مکمل مدد کی جائے، اور آپس میں ایک دوسرے کے ساتھ اور انتظامیہ کے ساتھ تعاون کرنے کے مواقع فراہم کیے جائیں تاکہ وہ اسکول کے اہداف کو پورا کرنے کے لیے اپنے علم اور صلاحیتوں کو بہتر بنائیں۔ اسکول قائدین کو بھی اساتذہ کی نگرانی کرنے اور جائزہ لینے کی ضرورت ہے، تاکہ ان کو مثبت انداز میں رائے دی جاسکے کہ کس طرح بہتری لائی جا سکتی ہے۔ اسکول قائدین اساتذہ کو ایسے تمام وسائل، مواقع اور رہنمائی فراہم کرنے کے ذمہ دار ہیں جن کی انہیں اسکول کے اہداف کو پورا کرنے میں ضرورت ہوتی ہے۔ </p>
				</ol>
			</div>

			<li><b>Creating an Enabling Environment and Positive Culture</b></li>
			<p>The school leader needs to create systems so that the school functions well. This includes building relationships by being accessible to teachers, staff, and students.  It also includes motivating teachers so that they love their jobs and want to do better, and setting up processes that help everyone achieve the school’s goals. This helps create a culture where everyone cooperates and works together towards the same goals. The school culture becomes positive and supportive, not negative or complacent. Students and teachers both benefit from a positive school culture where they feel valued and cared for. This motivates them to work harder and try and do better. </p>

			<div className="urdu">
				<ol start={3}>
					<li><b>ایک عمدہ ماحول اور مثبت ثقافت کی تشکیل </b></li>
					<p>	اسکول قائد کو ایسا سسٹم بنانے کی ضرورت ہے جس سے اسکول بہتر طریقے سے چل سکے۔ اس میں اساتذہ، عملہ اور طلباء تک قابلِ رسائی رہ کر تعلقات استوار کرنا شامل ہے۔ اس میں اساتذہ کی حوصلہ افزائی بھی شامل ہے تاکہ وہ اپنی ملازمت سے محبت کریں اور بہتر کام کریں، اور ایسے عمل مرتّب کریں جو مثبت نتائج حاصل کرنے میں ہر ایک کو مدد فراہم کریں۔ اس سے ٹیم ورک کی ثقافت پیدا کرنے میں مدد ملتی ہے جہاں ہر شخص ایک دوسرے کے ساتھ تعاون کرتا ہے اور سب مل کر ایک ہی مقصد کے لئے کام کرتے ہیں۔ اس سے اسکول کی ثقافت منفی یا مطمئن نہیں بلکہ مثبت اور معاون ثابت ہوتی ہے۔ طلباء اور اساتذہ دونوں ہی ایک مثبت اسکول ثقافت سے فائدہ اٹھاتے ہیں جہاں ان کو محسوس ہوتا ہے کہ ان کی قدر ہے اور ان کا خیال رکھا جاتا ہے۔ اس سے ان کو رغبت ملتی ہے کہ وہ مزید محنت کریں اور بہتر کام کرنے کی کوشش کریں۔ </p>
				</ol>
			</div>
		</ol>

		<p>By building a system with clear goals, providing lots of support, and being organized, a school leader can impact student achievement by enhancing teacher capability and motivation. In the end, a school is effective not only when it is producing good results, but when everyone who is part of the system is happy to be present. </p>
		<div className="urdu">
			<p> ایسا سسٹم بنایا جائے جہاں اہداف واضح ہوں، ہر کوئی دوسرے کے ساتھ تعاون کرے اور سب منظم رہیں۔ ایسے سسٹم کے ساتھ ساتھ  اسکول کے رہنما کو اساتذہ کی صلاحیت اور حوصلہ افزائی میں بھی اضافہ کرنا ہوتا ہے تا کہ وہ طلباء کی کامیابی کو متاثر کر سکے۔ آخر میں، ایک اسکول نہ صرف اس صورت میں مؤثر ہے جب وہ اچھے نتائج برآمد کر رہا ہے، بلکہ اس وقت بھی جب سسٹم کا حصہ بننے والے ہر شخص کو ٹیم کا حصہ بننے اور اس ٹیم میں اپنے موجود ہونے پر خوشی ہو۔</p>
		</div>
	</div>

</div>