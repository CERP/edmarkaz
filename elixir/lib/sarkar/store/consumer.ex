defmodule EdMarkaz.Store.Consumer do
	use GenServer
	...
	def handle_call({:load, id}, _from, state) do
		case EdMarkaz.DB.Postgres.query(
			EdMarkaz.DB,
			"SELECT sync_state from consumers where id=$1", [id]) do
				{:ok, %Postgrex.Result{num_rows: 0}} -> {:reply, {%{}, %{}}, state}
				{:ok, resp} ->
					[[sync_state]] = resp.rows

					case EdMarkaz.DB.Postgres.query(EdMarkaz.DB, "SELECT path, value, time, type, client_id FROM platform_writes WHERE id=$1 ORDER BY time desc limit $2", [id, 50]) do
						{:ok, writes_resp} ->
	...
	def handle_call({:save, id, sync_state}, _from, state) do
		...
		case EdMarkaz.DB.Postgres.query(
			EdMarkaz.DB,
			"INSERT INTO consumers (id, sync_state) VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET sync_state=$2",
			[id, sync_state]) do
