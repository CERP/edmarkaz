defmodule Mix.Tasks.Migrate do
	...
	def run(["assign_max_students"]) do
		...
		case Postgrex.query(EdMarkaz.School.DB, "SELECT school_id, db from backup", []) do
			{:ok, res} -> 
				res.rows 
	...
	def assign_max_students(school_id, limit) do
		...
		EdMarkaz.School.sync_changes(school_id, "backend-task", changes, :os.system_time(:millisecond))
		...
		case Postgrex.query(EdMarkaz.School.DB, "SELECT school_id, db from backup", []) do
			{:ok, res} ->
				...
				|> Enum.each(fn ([school_id, school_db]) ->
					...
					case Postgrex.query(EdMarkaz.School.DB, "INSERT INTO backup(school_id, db) VALUES ($1, $2) ON CONFLICT(school_id) DO UPDATE SET db=$2", [school_id, next_school]) do
	...
	defp start_school(school_id) do
		case Registry.lookup(EdMarkaz.SchoolRegistry, school_id) do
			[{_, _}] -> {:ok}
			[] -> DynamicSupervisor.start_child(EdMarkaz.SchoolSupervisor, {EdMarkaz.School, {school_id}})
		end
	end
end